...
const UUID = process.env.UUID || 'c35bbaff-6a5e-4060-a146-78d280d081ff';  // 在不同的平台运行了v1哪吒请修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'kata.lio.dpdns.org';           // argo固定隧道域名,留空即使用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiMTI4NjMxYWM0Nzg2ODI3MTk1MmFkYjllOTQ3YWQ2YTMiLCJ0IjoiZDc0NTZhMmMtNWZiYi00OGU5LWE5YTAtYmE2YTMzZTI5NGQzIiwicyI6IllXWm1NREF3TlRRdE1Ua3dZeTAwTmpWaUxUa3dPR1l0T0RCa04yTmlZakUzT1dObCJ9';               // argo固定隧道token或json,留空即使用临时隧道
const ARGO_PORT = process.env.ARGO_PORT || 32323;             // argo固定隧道端口,使用token需在cloudflare控制台设置和这里一致，否则节点不通
const S5_PORT = process.env.S5_PORT || '';                   // socks5端口，支持多端口的可以填写，否则留空
const TUIC_PORT = process.env.TUIC_PORT || '';               // tuic端口，支持多端口的可以填写，否则留空
const HY2_PORT = process.env.HY2_PORT || '20306';                 // hy2端口，支持多端口的可以填写，否则留空
const ANYTLS_PORT = process.env.ANYTLS_PORT || '';           // AnyTLS端口，支持多端口的可以填写，否则留空
const REALITY_PORT = process.env.REALITY_PORT || '20306';         // reality端口，支持多端口的可以填写，否则留空
